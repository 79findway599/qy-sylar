...
void do_io_read() {
    ...
    // 这里例子就是服务端发大于10个字节这里的回调函数do_io_read只能读10个字节，如果不把事件加进去，就会丢失包数据
...
void test_io() {
    ...
    SYLAR_LOG_INFO(g_logger) << "test_io begin";
    ...
    SYLAR_LOG_INFO(g_logger) << "test_io end";
}
